...
def t_pad(dev):
    ...
    # v2.1: PAD off is the LOUDER state, so back off 12 dB before the A/B.
    # v2.0 autolevelled with PAD on and then clipped when it came off,
    # which would have under-reported the difference.
    master = max(MASTER_MIN, master // 4)
    print("   backing off to master %d (%+.1f dB) for PAD-off headroom"
          % (master, db_of_master(master)))
    dev.master(master)
    dev.gain(30)
    out = {}
